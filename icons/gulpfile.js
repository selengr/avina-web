const kebabToCamelCaseMap = {
  ...
      // ===== بخش جدید: اضافه کردن مقادیر مورد نیاز برای تگ path =====
  ...
              })
          )
          // ===== بخش جدید: به‌روزرسانی regex جهت شامل کردن مقادیر اضافه شده =====
          .pipe(
              replace(
                  /(\bclip-rule\b|\bfill-rule\b|\bstroke-width\b|\bstroke-linecap\b|\bstroke-linejoin\b|\bstroke-miterlimit\b|\bfill-opacity\b|\bstroke-dasharray\b|\bstroke-dashoffset\b)/g,
                  (match) => kebabToCamelCaseMap[match] || match
              )
          )
          .pipe(replace(/<\/?svg(.*?)>/g, ''))
